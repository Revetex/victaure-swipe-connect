...
export const generateBusinessVCardPDF = async (profile: UserProfile): Promise<string> => {
  try {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(24);
    doc.text(profile.company_name || 'Business Profile', 20, 30);
    
    doc.setFontSize(16);
    doc.text(profile.industry || '', 20, 45);
    
    // Company Information Section
    doc.setTextColor(31, 41, 55);
    doc.setFontSize(14);
    doc.text('Company Information', 20, 80);
    ...
    // Generate the PDF blob
    const pdfBlob = doc.output('blob');
    const storageFileName = `business_${profile.id}_${Date.now()}.pdf`;

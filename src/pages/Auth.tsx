...
export default function Auth() {
  ...
  const handleSendMessage = () => {
    ...
    if (userInput.trim()) {
      ...
      setUserInput("");
      setTimeout(() => {
        const responses = ["Je comprends votre question. Pour y répondre pleinement, je vous invite à créer un compte. Cela me permettra de mieux vous accompagner.", "Excellente question ! Pour accéder à toutes les fonctionnalités et obtenir une réponse détaillée, je vous suggère de vous connecter.", "Je vois que vous êtes intéressé ! Connectez-vous pour découvrir toutes les possibilités que nous offrons."];
  ...
  return <div className="min-h-screen flex flex-col bg-[#1B2A4A] relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full px-4 py-8 sm:p-6 lg:p-8 relative z-10">
        <motion.div className="w-full max-w-xl mx-auto space-y-8 sm:space-y-12" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.5
          ...
          <div className="flex flex-col items-center justify-center space-y-6">
            ...
            <div className="w-full glass-panel rounded-xl p-4 border-2 border-[#222] shadow-[0_0_0_1px_rgba(100,181,217,0.1),0_4px_12px_rgba(0,0,0,0.3)]">
              ...
              <div ref={chatContainerRef} className="space-y-4 h-[400px] overflow-y-auto flex flex-col-reverse" style={{
              scrollBehavior: 'smooth'
            }}>
                <div className="flex flex-col-reverse">
                  <AnimatePresence mode="popLayout">
                    {visibleMessages.map((message, index) => <motion.div key={`user-${index}`} initial={{
                    opacity: 0,
                    y: 20
                  }} animate={{
                    opacity: 1,
                    ...
                    {autoMessages.map((message, index) => <motion.div key={`auto-${index}`} initial={{
                    opacity: 0,
                    y: 20
                  }} animate={{
                    opacity: 1,

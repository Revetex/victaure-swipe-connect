...
export function AppHeader({
  ...
  return <>
      ...
      opacity: 1,
      y: 0
    }} className="fixed top-0 left-0 right-0 z-[100] flex flex-col h-auto items-center justify-between px-4 bg-[#1A1F2C] border-b border-white/5 shadow-[0_2px_10px_rgba(0,0,0,0.2)]" style={{
            ...
            <div className="flex items-center gap-3">
              ...
              <Dialog open={showChat} onOpenChange={setShowChat}>
                <DialogContent className="max-w-3xl w-[95vw] h-[85vh] p-0 bg-[#1A1F2C] border border-[#64B5D9]/20 rounded-2xl mx-auto my-auto overflow-hidden">
                  <VictaureChat maxQuestions={user ? undefined : 3} context="Tu es Mr. Victaure, un assistant professionnel spécialisé dans l'emploi. Tu aides les utilisateurs à trouver du travail et à améliorer leur carrière." onMaxQuestionsReached={() => {
                  setShowChat(false);
                }} />
                </DialogContent>
              </Dialog>
          ...
          <div className="lg:hidden w-full pb-4">
            <div className="grid grid-cols-[1fr,auto] gap-2">
              <div className="relative">
                ...
                <Input type="text" value={searchQuery} onChange={handleSearch} onKeyPress={handleKeyPress} placeholder="Rechercher un emploi..." className="h-10 pl-10 pr-4 w-full bg-white/5 border-white/10 text-white placeholder:text-white/40 focus:bg-white/10 transition-colors" />
              </div>
              <Button variant="outline" className="h-10 px-4 bg-white/5 border-white/10 text-white hover:bg-white/10">
      ...
      y: -10
          ...
          <div className="w-full max-w-3xl mx-auto py-4">
            <div className="bg-[#1B2A4A] rounded-lg shadow-xl border border-[#64B5D9]/10 p-4">
              {isSearching ? <div className="flex items-center justify-center text-white/60 py-4">
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-3"></div>
                  Recherche en cours...
                </div> : searchQuery ? searchResults.length > 0 ? <div className="space-y-4">
                    {searchResults.map(job => <div key={job.id} className="p-4 bg-white/5 hover:bg-white/10 rounded-lg transition-colors cursor-pointer">
                  ...
                  </div> : <div className="text-white/60 py-4 text-center">

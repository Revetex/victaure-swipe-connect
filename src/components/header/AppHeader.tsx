  ...
  return <div className="fixed top-0 left-0 right-0 z-50">
      ...
      y: 0
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={toggleMobileMenu} className="lg:hidden text-white hover:bg-white/20 border-2 border-white/40">
            <Menu className="h-4 w-4" />
          </Button>
          
          <div className="flex items-center gap-3 justify-between relative z-50">
            <img src="/lovable-uploads/1af16883-f185-44b3-af14-6740c1358a27.png" alt="Victaure Logo" className="h-9 w-9 object-contain" />
        ...
        <div className="flex items-center gap-4">
          {totalJobs !== undefined && <p className="text-xs text-white/80 border-2 border-white/40 px-3 py-1.5 rounded-full">
              {totalJobs} offres disponibles
            </p>}

          <Button onClick={handleAssistantRequest} className="group relative overflow-hidden bg-white/10 hover:bg-white/20 text-white font-medium transition-all duration-300 border-2 border-white/40">

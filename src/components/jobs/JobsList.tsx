...
import { JobPosting } from "@/types/business";
...
export function JobsList({ jobs, isLoading, onEdit, onDelete, onView }: JobsListProps) {
  if (isLoading) {
    return (
        ...
        <Card key={job.id}>
          ...
          <CardFooter className="text-sm text-muted-foreground">
            <div className="flex justify-between w-full">
              <span>
                Publié {formatDistance(new Date(job.created_at), new Date(), { locale: fr })}
              </span>
              <span className={`capitalize ${job.status === 'published' ? 'text-green-600' : 'text-yellow-600'}`}>
                {job.status}
              </span>
            </div>
          </CardFooter>

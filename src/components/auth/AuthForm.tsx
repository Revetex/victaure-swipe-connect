  ...
  return <motion.div initial={{
    ...
    duration: 0.5
      ...
      <div className="flex flex-col gap-6">
        <div className="p-6 bg-[#1A1F2C]/90 backdrop-blur-sm border border-[#64B5D9]/10 rounded-xl shadow-lg relative">
          <Tabs defaultValue="login" value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-3 bg-[#1B2A4A]/20 mb-6 py-0 my-[8px]">
              ...
              <TabsTrigger value="signup" className={`text-[#F2EBE4]/80 transition-all py-3 ${activeTab === 'signup' ? 'bg-[#64B5D9]/20 text-[#64B5D9]' : ''}`}>
                Inscription
              </TabsTrigger>
              <TabsTrigger value="business" className={`text-[#F2EBE4]/80 transition-all py-3 ${activeTab === 'business' ? 'bg-[#64B5D9]/20 text-[#64B5D9]' : ''}`}>
                Entreprise
              </TabsTrigger>
            </TabsList>
        ...
        {/* Quick help section */}
        <motion.div animate={{
        height: showExtendedHelp ? 'auto' : '46px'
      }} className="relative overflow-hidden">
          <Card className="p-3 border-[#64B5D9]/10 backdrop-blur-sm py-0 bg-transparent px-[8px]">
            <div className="flex items-center justify-between cursor-pointer" onClick={() => setShowExtendedHelp(!showExtendedHelp)}>
              <div className="flex items-center gap-2">
                <Zap className="h-5 w-5 text-[#64B5D9]" />
                <h3 className="font-medium text-[#F2EBE4]">
                  {activeTab === 'login' ? "Problèmes de connexion ?" : activeTab === 'signup' ? "Guide d'inscription rapide" : "Avantages des comptes entreprise"}
                </h3>
              </div>
            ...
            {activeTab === 'login' && showExtendedHelp && <div className="mt-3 space-y-2 text-sm text-[#F2EBE4]/80">
                ...
                <div className="pt-2">
                  <Button variant="link" className="px-0 text-[#64B5D9] h-auto text-sm" onClick={() => setShowChat(true)}>
                    Discuter avec notre assistant →
                  </Button>
                </div>
              </div>}

            {activeTab === 'signup' && showExtendedHelp && <div className="mt-3 space-y-2 text-sm text-[#F2EBE4]/80">
            ...
            {activeTab === 'business' && showExtendedHelp && <div className="mt-3 space-y-2 text-sm text-[#F2EBE4]/80">
        ...
        <AnimatePresence>
          ...
          height: 0
              ...
              <AuthChat maxQuestions={3} context={activeTab === 'login' ? "Je suis l'assistant d'authentification. Je peux vous aider avec tout problème de connexion ou question liée à l'accès à votre compte." : activeTab === 'signup' ? "Je suis l'assistant d'inscription. Je peux vous aider à créer votre compte personnel et répondre à vos questions sur le processus d'inscription." : "Je suis l'assistant pour les comptes entreprise. Je peux vous expliquer les avantages des comptes professionnels et vous aider avec le processus d'inscription pour votre entreprise."} />
            </motion.div>}
        </AnimatePresence>
      </div>
    </motion.div>;

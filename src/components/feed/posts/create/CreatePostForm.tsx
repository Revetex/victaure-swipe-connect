...
export function CreatePostForm({
  ...
  return <motion.div className={cn("rounded-xl bg-[#1A1F2C] border border-white/10 transition-all duration-300", isExpandedInternal ? "p-3" : "p-2")} layout>
      {!isExpandedInternal ? <div className="flex items-center w-full gap-4 px-1">
          ...
          <Textarea value={newPost} onChange={e => onPostChange(e.target.value)} placeholder="Que souhaitez-vous partager ?" className="min-h-[100px] bg-white/5 border-white/10 text-white/90 placeholder:text-white/40 resize-none" />
          ...
          <motion.div layout className={cn("flex gap-4", isMobile ? "flex-col" : "flex-row justify-between items-center")}>
            ...
            <Button className="w-full sm:w-auto gap-2" disabled={isUploading || !newPost.trim()} onClick={onCreatePost}>

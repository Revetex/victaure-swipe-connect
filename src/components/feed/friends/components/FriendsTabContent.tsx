...
export function FriendsTabContent({
  ...
  return <ScrollArea className="h-[500px] rounded-lg bg-zinc-900/30 p-4 py-0 px-0">
      {displayedFriends.length > 0 ? <div className="space-y-2">
          {displayedFriends.map(friend => <FriendCard key={friend.id} friend={friend} />)}
        </div> : showOnlineOnly ? <div className="text-center py-8 text-zinc-400">

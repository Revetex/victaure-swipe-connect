...
export function DashboardStats() {
  const {
    ...
    queryFn: async (): Promise<DashboardStatsType> => {
      ...
      const {
        data: payments
      } = await supabase.from('payments').select('amount').eq('status', 'pending');
      const pendingPayments = payments?.reduce((sum, payment) => sum + Number(payment.amount), 0) || 0;

      // Fetch next job
      const {
        data: nextJobs
      } = await supabase.from('jobs').select('title').eq('status', 'open').order('created_at', {
        ascending: true
      }).limit(1);
      return {
  ...
  return <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" variants={containerVariants} initial="hidden" animate="visible">

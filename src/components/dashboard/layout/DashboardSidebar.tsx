      ...
      <div className="flex flex-col h-full bg-[#64B5D9] relative overflow-hidden border-r-2 border-black">
        ...
        <ScrollArea className="flex-1 px-4 py-2">
          <nav className="space-y-1.5">
            ...
            return <button key={item.id} onClick={() => onPageChange(item.id)} className={cn("w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm", "transition-all duration-200 border-2 shadow-sm", currentPage === item.id ? "bg-white/15 text-white font-medium border-black shadow-inner" : "text-white/90 hover:bg-white/10 hover:text-white border-black/20 hover:border-black")}>
                  <Icon className="h-3.5 w-3.5" />
                  <span className="font-medium">{item.name}</span>
                  {isNotificationsItem && unreadCount > 0 && <Badge variant="destructive" className="ml-auto min-w-[20px] h-5">
                      {unreadCount}
                    </Badge>}
                </button>;
          })}

...
export function AssistantMessage({ message, chatMessages, onSelectConversation }: AssistantMessageProps) {
  ...
  // If we have a direct message, render it
  if (message) {
    return (
      <motion.div
  ...
  // If we have chatMessages, render the chat preview
  if (chatMessages && onSelectConversation) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative flex gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-1">
            ...
            <p className="text-sm text-muted-foreground line-clamp-2">
              Cliquez pour démarrer une conversation avec votre assistant IA
            </p>
            <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
              <Clock className="h-3 w-3" />
              <span>En ligne</span>

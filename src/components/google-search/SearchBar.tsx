...
export function SearchBar({
  ...
  return <form onSubmit={onSearch} className="flex gap-2">
      <Input type="text" placeholder="Rechercher des offres d'emploi..." value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="" />
      <Button type="submit" disabled={isSearching} className="h-12 px-4">
        <SearchIcon className="h-4 w-4" />
      </Button>
      <Button type="button" onClick={generateRandomSearch} className="h-12 px-4" variant="outline" title="Générer une recherche basée sur votre profil">

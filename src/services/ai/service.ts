...
// New functions for form assistance and job recommendations
export async function getFormAssistance(formType: string, currentValues: any): Promise<string> {
  try {
    const profile = await getUserProfile();
    const prompt = `En tant qu'assistant pour le formulaire ${formType}, avec les valeurs actuelles ${JSON.stringify(currentValues)} et le profil utilisateur ${JSON.stringify(profile)}, quelle est la prochaine étape ou suggestion ?`;
    
    return await generateAIResponse(prompt);
  } catch (error) {
    console.error('Erreur lors de l\'assistance au formulaire:', error);
    throw error;
  }
}

export async function getJobRecommendations(): Promise<any[]> {
  try {
    const profile = await getUserProfile();
    ...
    // Filter and score jobs based on profile match
    const scoredJobs = jobs.map(job => ({
      ...job,
      score: calculateJobMatch(job, profile)
    })).sort((a, b) => b.score - a.score);

    return scoredJobs.slice(0, 5); // Return top 5 matches
  } catch (error) {
...
function calculateJobMatch(job: any, profile: UserProfile): number {
  let score = 0;
  ...
  score += (matchingSkills.length / jobSkills.size) * 50;

  // Location match
  if (job.location === profile.city) {
    score += 25;
  } else if (job.location === profile.state) {
    score += 15;
  }

  // Experience level match (simplified)
  if (job.experience_level === 'junior' || job.experience_level === 'entry') {
    score += 25;
  }

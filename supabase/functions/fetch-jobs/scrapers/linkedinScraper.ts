...
export class LinkedInScraper implements JobScraper {
  async scrape(url: string): Promise<ScrapedJob[]> {
    try {
      ...
      const jobListings = html.match(/<div class="job-card-container".*?<\/div>/gs) || [];

      for (const listing of jobListings) {
        const title = listing.match(/class="job-card-list__title">(.*?)<\//) || ['', ''];
        const company = listing.match(/class="job-card-container__company-name">(.*?)<\//) || ['', ''];
        const location = listing.match(/class="job-card-container__metadata-item">(.*?)<\//) || ['', ''];
        const salary = listing.match(/class="job-card-container__salary-info">(.*?)<\//) || ['', ''];
        const posted = listing.match(/class="job-card-container__posted-date">(.*?)<\//) || ['', ''];
        const url = listing.match(/href="(.*?)"/) || ['', ''];
        const id = url[1].split('/').pop() || '';
